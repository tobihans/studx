...
export default defineConfig({
  ...
  plugins: [
    ...
    plugin(({ addUtilities, theme, variants }) => {
      const newUtilities: { [k: string]: unknown } = {};

      Object.entries(theme("colors")).map(([name, value]: [string, string]) => {
        if (name === "transparent" || name === "current") return;

        const color = value[300] ? value[300] : value;
        const { h, s, l } = hex2hsl(color);
        const hsla = `hsla(${h},${s}%,${l}%,0.45)`;

        newUtilities[`.shadow-outline-${name}`] = {
          "box-shadow": `0 0 0 2.5px ${hsla}`,
        };
      });

      addUtilities(newUtilities as any, variants("boxShadow"));
    }),
  ],
});
